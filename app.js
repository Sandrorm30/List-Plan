...
function toggleEditMode() {
  ...
  renderTable(window._lastTableData);
}
...
function renderTable(data) {
  ...
  const ths = data.headers.map(h => `<th title="${esc(h)}">${esc(h)}</th>`).join('');
  const PAGE_SIZE = 50;
  const pageOffset = ((data.page || 1) - 1) * PAGE_SIZE;
  const trs = data.rows.map((row, ri) => {
    const sheetRow = pageOffset + ri + 2;
    const tds = row.map((c, ci) => {
      if (editMode) {
        return `<td><span class="cell-view" onclick="startEdit(this,${sheetRow},${ci+1})">${esc(c)}</span></td>`;
      }
  ...
  content.innerHTML = `
    <div class="table-wrap">
      <table>
        ...
        <tbody>${trs || `<tr><td colspan="${data.headers.length}" style="text-align:center;color:var(--muted)">Nenhum resultado.</td></tr>`}</tbody>
      </table>
    </div>`;
}
...
async function exportCSV() {
  ...
  if (!data || !data.headers.length) { alert('Sem dados.'); return; }
...
function exportPDF() {
  ...
  const tableHtml = `<table>
    ...
    <tbody>${data.rows.map(r=>`<tr>${r.map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('')}</tbody>
  </table>`;
  ...
  win.document.write(`<!doctype html><html><head><meta charset="utf-8"/>
    ...
    <style>
      ...
      th{background:#1a73e8;color:#fff;padding:7px 10px;text-align:left;font-size:11px;text-transform:uppercase}
    ...
    <h2>${title}</h2>
    <p>Gerado em ${date} · ${data.total} linhas · Página ${data.page} de ${data.pages}</p>
    ${tableHtml}
